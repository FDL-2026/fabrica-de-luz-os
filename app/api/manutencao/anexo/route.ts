...
// Proxy PÚBLICO de foto de manutenção, para o cliente que abre o link do
// shopping (/chamado/<token>). Só serve a foto se ela pertence a uma
// manutenção do projeto daquele token (autorização pela RPC).
export async function GET(request: NextRequest) {
  const token = request.nextUrl.searchParams.get("token") ?? "";
  const fileId = request.nextUrl.searchParams.get("fileId") ?? "";
  const querThumb = request.nextUrl.searchParams.get("thumb") === "1";

  if (!token || !fileId || !/^[\w-]+$/.test(fileId)) {
  ...
  const { data, error } = await supabase.rpc("fdl_anexo_manutencao_publico", {
    p_token: token,

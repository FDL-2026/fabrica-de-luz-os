...
// Proxy de foto de chamado para o MONTADOR (que acessa por PIN, sem sessão
// autenticada). A autorização é feita pelo vínculo montador ↔ projeto via RPC;
// só então o arquivo é servido a partir do Google Drive.
export async function GET(request: NextRequest) {
  ...
  if (!usuarioId || !fileId || !/^[\w-]+$/.test(fileId)) {
  ...
  const { data, error } = await supabase.rpc("fdl_anexo_chamado_montador", {

-- ============================================================
-- NATAL DO BEM / TEMPLATE "MUNDOS" — colunas novas + RPC estendida
-- Rode no SQL Editor do Supabase (teste na preview antes da produção).
--
-- 1) Colunas aditivas (idempotentes) para o template por "mundos":
--    - projetos.is_chave           -> destaca o projeto-chave (ex.: Natal do Bem)
--    - etapas_projeto.*            -> id do espaço, contrato, status de produção,
--                                     datas programadas e % de avanço de referência
--    - ordens_servico.*           -> contrato e datas programadas (contrato)
--
--    As datas "previstas" (inicio_previsto/termino_previsto) continuam sendo a
--    PROPOSTA (base do SLA). As "programadas" guardam a linha de base do contrato.
--
-- 2) confirmar_importacao_cronograma estendida para gravar essas colunas.
--    Comportamento legado preservado, incluindo o "blindar progresso": ao
--    ATUALIZAR uma OS existente, progresso e execução real NÃO são tocados.
--    is_chave é gravado pela rota /importar/confirmar (não por esta função).
-- ============================================================

alter table public.projetos
  add column if not exists is_chave boolean not null default false;

alter table public.etapas_projeto
  add column if not exists id_espaco text,
  ...
  add column if not exists termino_programado date,
  add column if not exists progresso_referencia text;
  ...
  loop
    ...
    if v_etapa_id is null then
      ...
      values (
        ...
        nullif(v_etapa->>'idEspaco', ''),
        nullif(v_etapa->>'contrato', ''),
        nullif(v_etapa->>'statusProducao', ''),
        public.fdl_parse_date(v_etapa->>'inicioProgramado'),
        public.fdl_parse_date(v_etapa->>'terminoProgramado'),
        nullif(v_etapa->>'progressoReferencia', '')
      )
      ...
      set
        ...
        id_espaco = coalesce(nullif(v_etapa->>'idEspaco', ''), ep.id_espaco),
        contrato = coalesce(nullif(v_etapa->>'contrato', ''), ep.contrato),
        status_producao = coalesce(nullif(v_etapa->>'statusProducao', ''), ep.status_producao),
        inicio_programado = coalesce(public.fdl_parse_date(v_etapa->>'inicioProgramado'), ep.inicio_programado),
        termino_programado = coalesce(public.fdl_parse_date(v_etapa->>'terminoProgramado'), ep.termino_programado),
        progresso_referencia = coalesce(nullif(v_etapa->>'progressoReferencia', ''), ep.progresso_referencia),
        atualizado_em = now()
  ...
  loop
    ...
    if v_os_id is null then
      ...
      values (
        ...
        nullif(v_os->>'contrato', ''),
        public.fdl_parse_date(v_os->>'inicioProgramado'),
        public.fdl_parse_date(v_os->>'terminoProgramado')
      );
      ...
      set
        ...
        contrato = coalesce(nullif(v_os->>'contrato', ''), os.contrato),
        inicio_programado = coalesce(public.fdl_parse_date(v_os->>'inicioProgramado'), os.inicio_programado),
        termino_programado = coalesce(public.fdl_parse_date(v_os->>'terminoProgramado'), os.termino_programado)
      where os.id = v_os_id;
